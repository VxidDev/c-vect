...
| * AppendVec(void vec, void item)**                         | AppendVec(vec, &value)                | Append an element to the end of the vector. Automatically resizes (×2) as needed.                                                          |
| * FreeVec(void vec)*                                       | FreeVec(vec)                          | Free all allocated memory associated with the vector.                                                                                      |
| * GetLastVec(void vec)*                                    | ptr = GetLastVec(vec)                 | Get a pointer to the last element, or NULL if the vector is empty.                                                                         |
| * PopVec(void vec)*                                        | ptr = PopVec(vec)                     | Remove and return a pointer to the last element. Returns NULL if the vector is empty.                                                      |
