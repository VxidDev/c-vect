...
int main(int argc , char **argv) {
    ...
    SetItemVec(IntVec, &(int){33}, 1);
    printf("Item: %i (expected 33)\n" , *(int*)GetItemFromVec(IntVec , 1));
    FreeVec(IntVec);
    ...
    SetItemVec(FloatVec, &(float){33.33}, 1);
    printf("Item: %.2f (expected 33.33)\n" , *(float*)GetItemFromVec(FloatVec , 1));
    FreeVec(FloatVec);
    ...
    SetItemVec(CharVec, "g", 1);
    printf("Item: %c (expected 'g')\n" , *(char*)GetItemFromVec(CharVec , 1));
    FreeVec(CharVec);
