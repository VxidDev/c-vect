...
int main(int argc , char **argv) {
    ...
    InsertVec(IntVec, &(int){17}, 1);
    printf("Item: %i (expected 17)\n" , *(int*)GetItemFromVec(IntVec , 1));
    FreeVec(IntVec);
    ...
    printf("\nItem: %.2f (expected 2.2)\n" , *(float*)GetItemFromVec(FloatVec , 1));
    InsertVec(FloatVec, &(float){17.17}, 1);
    printf("Item: %.2f (expected 17.17)\n" , *(float*)GetItemFromVec(FloatVec , 1));
    FreeVec(FloatVec);
    ...
    InsertVec(CharVec, "c", 1);
    printf("Item: %c (expected 'l')\n" , *(char*)GetItemFromVec(CharVec , 1));
    FreeVec(CharVec);
    ...
    InsertVec(StrVec, "ThisWorks", 1);
    printf("Item: %s (expected 'ThisWorks')\n" , *(char**)GetItemFromVec(StrVec , 1));
    FreeVec(StrVec);
